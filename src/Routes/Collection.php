...
use Surgio\Jsroute\Routes\Exceptions\ZeroRoutesException;
...
class Collection extends \Illuminate\Support\Collection
{
    ...
    protected function getRouteInformation(Route $route, $filter, $namespace)
    {
        ...
        $jsroute = Arr::get($route->getAction(), 'jsroute', null);
        ...
        switch ($filter) {
            case 'all':
                if($jsroute === false) return null;
                break;
            case 'only':
                if($jsroute !== true) return null;
                break;
        }
